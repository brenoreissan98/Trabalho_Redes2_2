...
class InterfaceDoUsuario(tkinter.Frame):
    def __init__(self, master=None):
        ...
        # variável auxiliar para saber se o frame de consulta está sendo mostrado
        self.mostrando_consulta = False
        # variável auxiliar para saber se o botao de encerrar ligação está renderizado ou não
        self.mostrando_botao_de_encerrar = False
        # variável auxiliar para saber se os botoes de tratar ligação está renderizado ou não
        self.mostrando_botoes_de_tratar_ligacao = False
        # variável auxiliar para saber se o botão de desistir de uma ligação está renderizado ou não
        self.mostrando_botao_de_desligar_ligacao = False
        # variável auxiliar para saber se o botão de ligar está renderizado
    ...
    def cria_container_do_form_do_usuario(self):
        ...
        self.label_nome_usuario = Label(
            self.container_nome_usuario, text="Nome", font=self.fonte)
        ...
        self.botar_de_submeter_form = Button(
            self.container_botao_submit, text="REGISTRAR", fg="white", bg="green", command=self.conectar_usuario)
  ...
  """
    ...
    def cria_mensagem_de_nome_ja_existente(self):
        ...
        self.label_mensagem_usuario_ja_registrado = Label(
            self.container_mensagem_usuario_ja_registrado, fg="red", text=f"NOME DE USUÁRIO JÁ REGISTRADO")
    ...
    def cria_label_para_usuario_conectado(self):
        self.container_para_label_do_nome_do_usuario_conectado = Frame(
            self.master)
        self.container_para_label_do_nome_do_usuario_conectado.pack(side=TOP)
        self.label_nome_do_usuario_conectado = Label(
            self.container_para_label_do_nome_do_usuario_conectado, text=f"BEM VINDO!\n {self.nome} ({self.ip}:{self.porta}))")
    ...
    def cria_formulario_de_consulta_de_usuario(self):
        self.container_do_formulario_de_consulta_de_usuario = Frame(
            self.master)
        self.container_do_formulario_de_consulta_de_usuario.pack(side=BOTTOM)
        self.label_formulario_de_consulta = Label(
            self.container_do_formulario_de_consulta_de_usuario, text="Consulta: ", font=self.fonte)
        self.label_formulario_de_consulta.pack(side=LEFT)
        self.formulario_de_consulta = Entry(
            self.container_do_formulario_de_consulta_de_usuario)
        self.formulario_de_consulta.pack(side=LEFT)
        self.botao_de_consultar = Button(self.container_do_formulario_de_consulta_de_usuario, text="CONSULTAR",
                                         bg="blue", fg="white", font=self.fonte, command=self.cria_tela_de_usuario_consultado)
    ...
    def cria_mensagem_usuario_ocupado(self):
        ...
        self.label_usuario_ocupado = Label(
            self.container_mensagem_usuario_ocupado, fg="red", text=f"USUÁRIO OCUPADO")
    ...
    def cria_botoes_de_tratar_ligacao(self):
        ...
        self.usuario_ligando_label = Label(
            self.container_tratar_ligacao, text=f"Recebendo ligação de : {usuario_ligando.nome}")
        self.usuario_ligando_label.pack(side=TOP)
        self.botao_de_aceitar_ligacao = Button(
            self.container_tratar_ligacao, text="ACEITAR", fg="white", bg="green", command=self.aceitar_ligacao)
        self.botao_de_aceitar_ligacao.pack(side=LEFT)
        self.botao_de_recusar_ligacao = Button(
            self.container_tratar_ligacao, text="RECUSAR", fg="white", bg="red", command=self.recusar_ligacao)
    ...
    def cria_botao_de_encerrar_ligacao(self):
        ...
        self.botao_encerrar_ligacao = Button(
            self.container_botao_encerrar_ligacao, text="ENCERRAR", fg="white", bg="red", command=self.encerra_ligacao)
    ...
    def encerra_ligacao(self):
        ...
        enredeco = (self.servidor_de_ligacao.usuario_destino.ip,
                    self.servidor_de_ligacao.usuario_destino.porta)
    ...
    def destroi_tela_de_usuario_consultado(self):
        self.mostrando_consulta = False
    ...
    def cria_tela_de_usuario_consultado(self):
        ...
        mensagem = pickle.dumps(
            {"operacao": "consultar", "data": self.nome_consultado})
        self.socket.sendall(mensagem)
        data = self.socket.recv(1024)
        ...
        if self.mostrando_consulta:
            self.destroi_tela_de_usuario_consultado()
        ...
        if self.usuario_consultado:
            ...
            self.label_usuario_consultado = Label(self.container_tela_de_usuario_consultado,
                                                  text=f"NOME: {usuario_consultado.nome}\nIP: {usuario_consultado.ip}\nPORTA: {usuario_consultado.porta}")
            self.label_usuario_consultado.pack(side=BOTTOM)
            self.mostrando_consulta = True
        else:
            self.container_tela_de_usuario_consultado = Frame(self.master)
            self.container_tela_de_usuario_consultado.pack(side=TOP)
            self.label_usuario_consultado = Label(
                self.container_tela_de_usuario_consultado, fg="red", text=f"USUÁRIO NÃO ENCONTRADO")
            self.label_usuario_consultado.pack(side=BOTTOM)
            self.mostrando_consulta = True
    ...
    def cria_botao_de_desconectar(self):
        ...
        self.botao_de_desconectar = Button(
            self.container_do_botao_de_desconectar, text="DESCONECTAR", fg="white", bg="red", command=self.desconecta)
    ...
    def cria_botao_de_ligar(self):
        ...
        self.botao_de_ligar = Button(self.container_do_botao_de_desconectar,
                                     text="LIGAR", fg="white", bg="green", command=self.ligar)
    ...
    def ligar(self):
        ...
        self.servidor_de_ligacao.liga_para_usuario(
            self.usuario_conectado, self.usuario_consultado)
        self.cria_botao_desligar_ligacao()
    ...
    def cria_botao_desligar_ligacao(self):
        ...
        self.botao_de_desligar = Button(
            self.container_botao_de_desligar, text="DESISTIR DA LIGAÇÃO", fg="white", bg="red", command=self.desligar_ligacao)
    ...
    def desligar_ligacao(self):
        ...
        self.servidor_de_ligacao.envia_mensagem(
            pickle.dumps(mensagem), endereco)
        self.destroi_botao_desligar_ligacao()
        self.cria_botao_de_ligar()
    ...
    def desconecta(self):
        ...
        if (resultado):
            ...
            if self.mostrando_mensagem_de_usuario_ocupado:
                self.destroi_mensagem_usuario_ocupado()

            if self.mostrando_botoes_de_tratar_ligacao:
                self.destroi_botoes_de_tratar_ligacao()

            if self.mostrando_botao_de_desligar_ligacao:
                self.destroi_botao_desligar_ligacao()

            if self.mostrando_botao_de_encerrar:
                self.destroi_botao_de_encerrar_ligacao()
