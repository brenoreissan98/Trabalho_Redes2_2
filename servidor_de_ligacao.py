...
class ServidorDeLigacao:
    ...
    def trata_convite(self, usuario_ligando):
        ...
        if not self.em_ligacao and not self.recebendo_ligacao:
            ...
            # Guarda qual é o usuário que está ligando
            self.usuario_destino = usuario_ligando
        else:
            print(f"OCUPADO")
            mensagem = {"operacao": "resposta_ao_convite", "data": "Ocupado"}
            self.envia_mensagem(pickle.dumps(mensagem),
                                (usuario_ligando.ip, usuario_ligando.porta))
    ...
    def grava_audio_e_envia(self, in_data, frame_count, time_info, status):
        try:
            ...
            # print(in_data)
            self.envia_mensagem(
                in_data, (self.usuario_destino.ip, self.usuario_destino.porta))
    ...
    def trata_resposta(self, resposta, enviar_resposta=False):
        if resposta == True:
            if enviar_resposta:
                ...
                self.envia_mensagem(pickle.dumps(
                    mensagem), (self.usuario_destino.ip, self.usuario_destino.porta))
            ...
            self.stream_input = self.audio_input.open(format=self.FORMAT, channels=self.CHANNELS, rate=self.RATE,
                                                      input=True, frames_per_buffer=self.CHUNK, stream_callback=self.grava_audio_e_envia)
            self.audio_output = pyaudio.PyAudio()
            self.stream_output = self.audio_output.open(
                format=self.FORMAT, channels=self.CHANNELS, rate=self.RATE, output=True, frames_per_buffer=self.CHUNK)
